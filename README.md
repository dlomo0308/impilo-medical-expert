...
```bash
conda create python=3.8 -n impilo
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate impilo
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///impilo` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE impilo WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE impilo TO ;
```
...
ERROR:  database "impilo" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists impilo
